	...
	public static class Generator
	{
		...
		private static IEnumerable<ICrossBoard> GenerateCrossWords(ICrossBoard board, ICrossDictionary dictionary, string? puzzle, CancellationToken cancellationToken)
		{
			if (puzzle != null)
			{
				...
				var placer = new PuzzlePlacer(board, puzzle);
				var allPossiblePlacements = placer.GetAllPossiblePlacements(dictionary);
				if (!allPossiblePlacements.Any())
				{
					Log.Error("No boards with puzzle '{0}' could be generated!", puzzle);
				}
				foreach (var boardWithPuzzle in allPossiblePlacements)
				{
					cancellationToken.ThrowIfCancellationRequested();
