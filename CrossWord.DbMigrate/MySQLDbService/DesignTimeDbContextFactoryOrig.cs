...
namespace CrossWord.DbMigrate.MySQLDbService
{
    public class DesignTimeDbContextFactoryOrig : IDesignTimeDbContextFactory<WordHintDbContextOrig>
    {
        ...
        public WordHintDbContextOrig CreateDbContext(string connectionString)
        {
            return CreateDbContext(connectionString, null);
        }

        public WordHintDbContextOrig CreateDbContext(string connectionString, Serilog.ILogger log)
        {
            var args = new string[] { $"ConnectionStrings:DefaultConnection={connectionString}" };
            return CreateDbContext(args, log);
        }

        public WordHintDbContextOrig CreateDbContext(string[] args, Serilog.ILogger log)
        {
            // set logging
